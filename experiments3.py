...
def embedding_lang(p_mod, p_rc, p_src, num_levels):
    rules = [
        (pcfg.Rule('S', ('NP1', 'V1')), 0.5),
        (pcfg.Rule('S', ('NP2', 'V2')), 0.5),


        (pcfg.Rule('NP', ('NP1')), 0.5),
        (pcfg.Rule('NP', ('NP2')), 0.5),


        (pcfg.Rule('NP1', ('N1',)), 1 - p_mod),
        (pcfg.Rule('NP1', ('N1', 'C', "S")), p_mod * p_rc * (1 - p_src)),
        (pcfg.Rule('NP1', ('N1', 'C', "V1", "NP")), p_mod * p_rc * p_src),
        (pcfg.Rule('NP1', ('N1', 'P', 'NP')), p_mod * (1 - p_rc)),

        (pcfg.Rule('NP2', ('N2',)), 1 - p_mod),
        (pcfg.Rule('NP2', ('N2', 'C', "S")), p_mod * p_rc * (1 - p_src)),
        (pcfg.Rule('NP2', ('N2', 'C', "V2", "NP")), p_mod * p_rc * p_src),
        (pcfg.Rule('NP2', ('N2', 'P', 'NP')), p_mod * (1 - p_rc)),
    ]
...
def verb_forgetting_conditions(num_levels=2,
    ...
    prefixes = [('N1', 'C', 'N2', 'V2'), ('N1', 'C', 'N2', 'C', 'N1', 'V1', 'V2')]
    ...
    def gen():
        for i in range(num_levels):
            ...
            for noisy_prefix, p in noisy_prefix_distro.dict.items():
                p_correct = npt[noisy_prefix].dict.get('V1', 0)
                p_incorrect = npt[noisy_prefix].dict.get("V2", 0)
