...
def embedding_lang(p_mod, p_rc, p_src, num_levels):
    rules = [
        (pcfg.Rule('S', ('NP', 'V')), 1),
        (pcfg.Rule('NP', ('N',)), 0.8 * (1 - p_mod)),
        (pcfg.Rule('NP', ('N', 'RC')), 0.8* p_mod * p_rc),
        (pcfg.Rule('NP', ('N', 'PP')), 0.8 * p_mod * (1 - p_rc)),


        (pcfg.Rule('NP', ('fact',)), 0.2 * (1 - p_mod)),
        (pcfg.Rule('NP', ('fact', 'RC')), 0.2* p_mod * 0.2),
        (pcfg.Rule('NP', ('fact', 'C', "S")), 0.2 * p_mod * (1 - 0.2)),

        (pcfg.Rule('NP', ('myth',)), 0.2 * (1 - p_mod)),
        (pcfg.Rule('NP', ('myth', 'RC')), 0.2* p_mod * 0.8),
        (pcfg.Rule('NP', ('myth', 'C', "S")), 0.2 * p_mod * (1 - 0.8)),
...
def verb_forgetting_conditions(num_levels=2,
    ...
    prefixes= []
    # exchange with "myth"
    prefixes.append(("fact", "C", "N", "V"))
    prefixes.append(("fact", "C", "N", "C", "N", "V", "V"))
