...
def embedding_lang(p_mod, p_rc, p_src, num_levels):
    p_sc_1 = 0.1 # this modulates the effect

    p_sc_2 = 0.9 # inkling
    rules = [
        (pcfg.Rule('S', ('NP', 'V')), 0.5),
        (pcfg.Rule('S', ('NP1', 'V')), 0.4), # fact
        (pcfg.Rule('S', ('NP2', 'V')), 0.1), # inkling
        (pcfg.Rule('NP', ('N',)), 1 - p_mod),
        (pcfg.Rule('NP', ('N', 'RC')), p_mod * p_rc),
        (pcfg.Rule('NP', ('N', 'PP')), p_mod * (1 - p_rc)),
        (pcfg.Rule('NP1', ('N1', 'C', "NP", "V")), p_sc_1),
        (pcfg.Rule('NP1', ('N1', 'PP')), (1 - p_sc_1)),
        (pcfg.Rule('NP2', ('N2', 'C', "NP", "V")), p_sc_2),
        (pcfg.Rule('NP2', ('N2', 'PP')), (1 - p_sc_2)),
        (pcfg.Rule('PP', ('P', 'NP')), 1),
...
def verb_forgetting_conditions(num_levels=2,
    ...
    prefixes = [('N1', 'C', 'N', 'V'), ('N1', 'C', 'N', 'C', 'N', 'V', 'V')]
    print(prefixes)
